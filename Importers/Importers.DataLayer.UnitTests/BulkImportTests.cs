...
namespace Importers.DataLayer.UnitTests
{
    ...
    public class BulkImportTests
    {
        ...
        [Test]
        public void TwoIdsAndTwoNonIdSqlStrings()
        {
            var tempTableFiller = new Mock<INpgTempTableFiller>();
            tempTableFiller.Setup(t => t.Fill("import_temp", "targettable", "id1, id2, field1, field2", It.IsAny<IEnumerable<string>>(), null));
            ...
            Assert.That(issuedSqlCommands[3], Is.EqualTo("update targettable t set population = it.population from import_temp it where it.id1 = t.id1 and it.id2 = t.id2 and it.population <> t.population"));
            Assert.That(issuedSqlCommands[4], Is.EqualTo("insert into targettable (id1, id2, population) select it.id1, it.id2, it.population from import_temp it left join targettable t using (id1, id2) where t.id1 is null"));
        }
    }
}
